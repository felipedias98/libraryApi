...
class ConfiguracionModel extends Query{
    ...
    {
        $sql = "SELECT p.id, p.id_estudiante, DATE_FORMAT(p.fecha_prestamo,'%d/%m/%Y') AS data_emprestimo, p.cantidad,p.estado, e.id, e.nombre, l.id, l.titulo FROM prestamo p INNER JOIN estudiante e ON p.id_estudiante = e.id INNER JOIN libro l ON p.id_libro = l.id WHERE p.fecha_devolucion < '$date' AND p.estado = 1";
        $res = $this->selectAll($sql);
        return $res;
    }
