...
class Usuarios extends Controller{
    ...
    public function validar()
    {
        ...
        if (empty($usuario) || empty($clave)) {
            ...
            if ($data) {
                ...
                $msg = array('msg' => 'Usuário ou senha incorreta!', 'icono' => 'warning');
            }
        }
        ...
        if (empty($usuario) || empty($nombre)) {
            ...
            if ($id == "") {
                if (!empty($clave) && !empty($confirmar)) {
                    if ($clave != $confirmar) {
                        $msg = array('msg' => 'A senha é obrigatória!', 'icono' => 'warning');
                    } else {
                        $data = $this->model->registrarUsuario($usuario, $nombre, $hash);
                        if ($data == "ok") {
                            $msg = array('msg' => 'Usuário cadastrado', 'icono' => 'success');
                        } else if ($data == "existe") {
                            $msg = array('msg' => 'Este usuário já existe!', 'icono' => 'warning');
                        } else {
                ...
                if ($data == "modificado") {
                    $msg = array('msg' => 'Usuario alterado!', 'icono' => 'success');
                }else {
    ...
    public function eliminar(int $id)
    {
        ...
        if ($data == 1) {
            $msg = array('msg' => 'Usuario dado baixa!', 'icono' => 'success');
        }else{
            $msg = array('msg' => 'Erro ao dar baixa no usuário!', 'icono' => 'error');
        }
        echo json_encode($msg, JSON_UNESCAPED_UNICODE);
        die();
    }
    public function reingresar(int $id)
    {
        ...
        if ($data == 1) {
            $msg = array('msg' => 'Usuário restaurado!', 'icono' => 'success');
        } else {
    ...
    public function cambiarPas()
    {
        if ($_POST) {
            ...
            if (hash("SHA256", $clave) == $user['clave']) {
                ...
                if ($data == "modificado") {
                    $msg = array('msg' => 'Senha alterada!', 'icono' => 'success');
                } else {
                    $msg = array('msg' => 'Erro ao alterar!', 'icono' => 'warning');
                }
            } else {
                $msg = array('msg' => 'Senha incorreta!', 'icono' => 'warning');
            }
            echo json_encode($msg, JSON_UNESCAPED_UNICODE);
            die();
        }
