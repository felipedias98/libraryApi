...
class Editorial extends Controller
{
    ...
    public function registrar()
    {
        ...
        if (empty($editorial)) {
            $msg = array('msg' => 'O nome é um campo requerido!', 'icono' => 'warning');
        } else {
            if ($id == "") {
                $data = $this->model->insertarEditorial($editorial);
                if ($data == "ok") {
                    $msg = array('msg' => 'Editora cadastrada com sucesso!', 'icono' => 'success');
                } else if ($data == "existe") {
                    $msg = array('msg' => 'Essa editora já existe!', 'icono' => 'warning');
                } else {
                    $msg = array('msg' => 'Erro ao cadastrar editora!', 'icono' => 'error');
                }
            } else {
                $data = $this->model->actualizarEditorial($editorial, $id);
                if ($data == "modificado") {
                    $msg = array('msg' => 'Editora alterada com sucesso!', 'icono' => 'success');
                } else {
                    $msg = array('msg' => 'Erro ao alterar editora!', 'icono' => 'error');
                }
            }
        }
    ...
    public function eliminar($id)
    {
        ...
        if ($data == 1) {
            $msg = array('msg' => 'Editora dado baixa com sucesso!', 'icono' => 'success');
        } else {
            $msg = array('msg' => 'Erro ao dar baixa na editora!', 'icono' => 'error');
        }
        echo json_encode($msg, JSON_UNESCAPED_UNICODE);
        die();
    }
    public function reingresar($id)
    {
        ...
        if ($data == 1) {
            $msg = array('msg' => 'Editora restaurada com sucesso!', 'icono' => 'success');
        } else {
            $msg = array('msg' => 'Erro ao restaurar editora!', 'icono' => 'error');
        }
        echo json_encode($msg, JSON_UNESCAPED_UNICODE);
        die();
    }
